<template>
    <div class="ecg-view-box-wrap" v-loading="reAnalystLoading" element-loading-text="重新AI分析中">
        ...
        <!--驳回意见弹出框-->
        <el-dialog title="驳回意见" :visible.sync="dialogReject" width="600px">
            <el-form :model="ruleForm" :rules="rules" ref="rejectForm">
                <el-form-item prop="rejectSuggestion">
                    <el-input type="textarea" class="rejectInput" placeholder="请填写驳回意见" v-model="ruleForm.rejectSuggestion"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="handleReject(0)">取 消</el-button>
                <el-button type="primary" @click="handleReject(1)">确 定</el-button>
            </div>
        </el-dialog>
...
<script>
    ...
    import Arrhythmia from './Arrhythmia.vue';
    ...
    import {EcgLoader} from '../common/ecg_loader';
    export default {
        name: 'EcgView',
        components: {
            DetailedPatientInfo,
        ...
        data() {
            var validatePass = (rule, value, callback) => {
                let patt = new RegExp("^[A-Za-z\\-\\u4e00-\\u9fa5]+$")
                if (value.length > 200) {
                    callback(new Error('长度不超过200个字符'));
                } else if (!patt.test(value)) {
                    callback(new Error('只能填写中文、英文和-'));
                } else {
                    callback();
                }
            };
...
//            },
        ...
        mounted() {
            ...
            //实例化缓存原始心电的类
            this.changeEcgLoader(new EcgLoader(this.report_id, {
                originalFrequency: this.samplingFrequency,
                cachedFrequency: this.samplingRate
            }));
            this.ecgLoader.setMaxMemoryMB(100);
            this.ecgLoader.setPreLoadCount(10);
        },
        watch: {
            currentDate() {
        ...
        methods: {
            ...
            handleReject(flag) {
                if (flag) {
                    this.$refs['rejectForm'].validate((valid) => {
                        if (valid) {
                            let params = {
                                ...
                                state: 'EDITING',
                                auditOpinions: this.ruleForm.rejectSuggestion
                            };
                    ...
                    this.$refs.ecgViewPage.addFewSecondsEvent();
                }
            },
            rejectChange() {
                console.log(this.rejectSuggestion)
            },
            ...
            cancelReport() {
                ...
                this.$refs.ecgViewPage.removeFewSecondsEvent();
            },
            /**
        ...
        beforeDestroy() {
            bus.$off();
            $('.tab-left').off();
            this.resetEcgViewModule(['gainSelected', 'selectMain', 'tagChangeState',
                'samplingFrequency','redrawState','jumpPosition','samplingRate']);
...
<style scoped lang="scss">
    ...
    .rejectInput /deep/ .el-textarea__inner{
        height: 150px;
    }
    .ecg-view-box {
