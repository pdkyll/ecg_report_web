...
// 客户端Block的缓存和Ajax交互
var Block = function (reportId, index, loadedCallback) {
...
EcgViewer.prototype.resetOffset = function (offset) {
    this.offset = -1;
};
...
/**
 * 清除可视block块数据，以便重新获取数据
 * 需求：翻转片段重新渲染视图，删除片段等
 */
EcgViewer.prototype.updateVisibleData = function () {
    this.visibleData = null;
    let firstUsedBlockIndex = Math.max(Math.floor((this.offset - this.beforeSize) / this.blockSize), 0);
    let lastUsedBlockIndex = Math.min(Math.floor((this.offset + this.viewSize + this.afterSize) / this.blockSize), this.blockCount - 1);
    for (let i = firstUsedBlockIndex; i <= lastUsedBlockIndex; i++) {
        this.blocks[i].data = null
    }
    this.moveTo(Math.floor(this.offset + 1));
};
