<template>
    <div class="ecg-fragments-wrapper" ref="ecg-fragments-wrapper" :style="{width:wrapperSize.width+'px'}" @click.stop>
        ...
        <div ref="flexPaper" class="flexPaperBoxItem" v-loading="pageLoading"
            ...
            <div ref="singlePaper" class="singlePaperItem" :data-pos="item.p" draggable="false"
                 v-for="(item, index) in showArr"
                 @dragenter="noDefault"
                 :style="{'height': canvasHeight+'px'}"
                 @dragover="noDefault">
                <canvas :ref="item.p+'scatter'" :data-pos="item.p" :width="canvasWidth+'px'"
        ...
        <ul class="tempPopup" ref="operatePopup" v-if="!isShowRight" v-show="isShowOperatePopup">
            ...
            <li @click="showTempPopup($event, 1)" :class="{'popupDisabled':selectArr.length === 0 || pageLoading}">操作当前心搏</li>
            <!--<li @click="showTempPopup($event, 2)" :class="{'popupDisabled':selectArr.length === 0 || pageLoading}">操作前一个心搏</li>-->
        </ul>
    </div>
</template>
...
//                this.selectArray = Util.deepCopy(this.sourceData).sort((a, b) => a - b);
                ...
                if (this.isSelectFirstBeat && this.selectArray.length !== 0 && this.isResetPage) {
                    this.selectArr = [0];
                    this.showPosition = this.selectArray[0].p
                } else {
                    this.showPosition = -2
                    this.selectArr = [];
                }
...
//                        });
            ...
            pageSelectAll() {
                ...
                this.showPosition = this.showArr[this.showArr.length - 1].p;
            ...
            addArr(index) {
                ...
                if (selectIndex === -1) {
                    ...
                    if (this.showArr[index - this.pageObj.start]) {
                        this.showPosition = this.showArr[index - this.pageObj.start].p;
                    }
                } else if (this.onShiftButton) {
                    this.selectArr.push(index);
                    this.selectArr.sort((a, b) => a - b);
            ...
            editBeatType(type, positions, updateMode) {
                ...
                let param = {beatType: type, positions: positions.map((v) => v.p)}
                if (this.isInTemp) {
...
//                        that.$message('请先选择起始卡片')
            ...
            getSelectPointEcg(changeBeat = true, redraw = false) {
                if (this.selectArray.length) {
                    ...
                    this.ecgLoader.getSlicesOfBeats(slicePositions, 512, 512,(slices)=>{
                ...
                this.selectArr.map(item => {
                    ...
                    if (index >= 0 && index < this.pageObj.limit) {
                        if (this.showArr[item - this.pageObj.start]) {
                            if (Number(itemPos) === this.showArr[item - this.pageObj.start].p) {
